...
template <template <class T> class OrderingStructure> animation filler::Fill(FillerConfig& config)
{
	...
	vector<vector<bool>> processed(config.img.height(), vector<bool>(config.img.width(), false));
	os.Add(config.seedpoint);
	processed[config.seedpoint.x][config.seedpoint.y] = true;

	while (!os.IsEmpty()) {
        PixelPoint current = os.Remove();
        RGBAPixel* currentPixel = config.img.getPixel(current.x, current.y);
        *currentPixel = config.picker->operator()(current);

        if (++framecount % config.frameFreq == 0) {
            anim.addFrame(config.img);
        }

        PixelPoint neighbors[4] = {
        ...
        for (PixelPoint& n : neighbors) {
            if (n.x >= 0 && n.x < config.img.width() && n.y >= 0 && n.y < config.img.height()) {
                if (!processed[n.y][n.x] && (*config.img.getPixel(config.seedpoint.x,config.seedpoint.y)).distanceTo(*config.img.getPixel(n.x,n.y)) < config.tolerance) {
                    os.Add(n);
                    processed[n.y][n.x] = true;
                }
            }
        }
    }
